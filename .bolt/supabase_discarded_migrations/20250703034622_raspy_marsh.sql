/*
  ...
  1. New Tables
    - `user_plans`
      ...
      - `stripe_subscription_id` (text, references stripe_subscriptions)
  ...
  3. Automation
    - Trigger to create free plan for new users
    - Trigger to update timestamps on plan changes
    - View for easy plan status checking with subscription details
*/

-- Create enum types for plan management
CREATE TYPE IF NOT EXISTS user_plan_type AS ENUM ('free', 'monthly');
CREATE TYPE IF NOT EXISTS user_plan_status AS ENUM ('active', 'inactive');
